#!/usr/bin/env python3
"""
Weather and Ballpark Factors Fetcher for Run Scored Prediction
Calculates run environment multipliers using weather, park factors, and stadium orientation

Pipeline Stage 3 of 4:
1. mlb_lineups_fetcher.py
2. run_metrics_fetcher.py
3. weather_ballpark_run_fetcher.py  ← THIS SCRIPT
...
class WeatherBallparkRunFetcher:
    """
    Fetches weather data and calculates run environment multipliers
    """
    ...
    def __init__(self, api_key: str):
        ...
        # MLB Stadium coordinates and Run-specific park factors
        self.stadiums = {
            # AL East
            'NYY': {
                'name': 'Yankee Stadium',
                ...
                'lat': 40.8296,
                'lon': -73.9262,
                'run_factor': 105,  # 5% above league average
                'elevation': 55,
                'surface': 'grass',
                'orientation': 75  # degrees from North
            },
            'BOS': {
                'name': 'Fenway Park',
                'city': 'Boston',
            ...
            'COL': {
                ...
                'lat': 39.7559,
                'lon': -104.9942,
                'run_factor': 122,  # Extreme hitter's park
                'elevation': 5200,
                'surface': 'grass',
                'orientation': 0
            },
        ...
        print("[INFO] Weather and Ballpark Run Fetcher initialized")
        print(f"[INFO] Tracking {len(self.stadiums)} MLB stadiums")

    def enrich_lineups_with_weather(self, input_file: str = "run_metrics_output.json",
                                    output_file: str = "final_integrated_run_data.json") -> Dict:
        """
        Final cascading enrichment: Add weather/ballpark data to run metrics
        """
        print("="*80)
        print("[INFO] FINAL RUN ENRICHMENT: Adding Weather & Ballpark Data")
        print("="*80)

        # Load lineup data with Run metrics
        if not os.path.exists(input_file):
            print(f"[ERROR] {input_file} not found")
            print("[INFO] Run run_metrics_fetcher.py first")
            return {}
        ...
        enriched_data['final_integration_info'] = {
            ...
            'data_sources': ['lineups', 'run_metrics', 'weather_ballpark'],
            'ready_for_run_prediction': True,
    ...
    def _add_weather_ballpark_data(self, lineup_data: Dict) -> Dict:
        """
        Add Run-specific weather and ballpark information to each game
        """
    ...
    def _get_game_weather_info(self, game_data: Dict) -> Optional[Dict]:
        """
        Get Run-specific weather and ballpark info for a single game
        """
        try:
            ...
            # Calculate Run-specific weather effects
            run_multiplier = self._calculate_run_multiplier(weather_data, stadium_info)
            ...
            weather_info = {
                'stadium': {
                    ...
                    'orientation': stadium_info['orientation']
                },
                ...
                'park_run_factor': stadium_info['run_factor'] / 100,
                'total_run_multiplier': run_multiplier
            }
    ...
    def _get_default_weather(self) -> Dict:
        ...
        """
        return {
            'temperature': 75.0,  # Average game temperature (optimal)
            'feels_like': 75.0,
    ...
    def _fetch_weather(self, lat: float, lon: float) -> Optional[Dict]:
        ...
        try:
            ...
            # Suppress detailed error message when using fallback
            return None

    def _calculate_run_multiplier(self, weather: Dict, stadium: Dict) -> float:
        """
        Calculate total run environment multiplier from weather + park factors

        Formula:
        total_run_multiplier = temp_factor × humidity_factor × pressure_factor ×
                               wind_factor × park_run_factor
        ...
        try:
            ...
            # Warm air carries ball further → more runs
            if temp > 75:
            ...
            # 2. HUMIDITY FACTOR
            # Low humidity = denser air → slightly fewer runs
            if humidity < 40:
                humidity_factor = 0.97
            elif humidity > 70:
                humidity_factor = 1.03
            else:
                humidity_factor = 1.0

            # 3. AIR PRESSURE / ALTITUDE FACTOR
            # Lower pressure = thinner air = more offense
            pressure_factor = 1 + ((1013 - pressure) / 20) * 0.02

            # 4. WIND FACTOR (with stadium orientation)
            stadium_orientation = stadium.get('orientation', 0)

            # Calculate angle difference between wind and stadium orientation
            angle_diff = (wind_direction - stadium_orientation + 180) % 360 - 180

            # Determine wind effect based on direction
            if -45 <= angle_diff <= 45:
                # Wind blowing out to center - helps offense
                wind_factor = 1 + (wind_speed * 0.004)
            elif 45 < angle_diff <= 90 or -90 <= angle_diff < -45:
                # Wind to power alleys - helps somewhat
            ...
            # Bound wind factor
            wind_factor = max(0.85, min(1.15, wind_factor))

            # 5. PARK RUN FACTOR
            # Statcast/FanGraphs park factor (100 = league average)
            park_run_factor = stadium.get('run_factor', 100) / 100
            ...
            # Cap between 0.75 and 1.35
            total_run_multiplier = max(0.75, min(1.35, total_run_multiplier))

            return round(total_run_multiplier, 3)

        except Exception as e:
            print(f"[ERROR] Error calculating run multiplier: {e}")
    ...
    def _display_integration_summary(self, data: Dict):
        """
        Display summary of final integrated Run data
        """
        print("\n" + "="*80)
        print("[INFO] FINAL RUN INTEGRATION SUMMARY")
        print("="*80)
        ...
        print(f"\nData Ready for Run Prediction: {data.get('final_integration_info', {}).get('ready_for_run_prediction', False)}")
        print("="*80)


def main():
    """
    Main function for Run weather/ballpark enrichment
    """
    # Use your OpenWeatherMap API key
    API_KEY = "e09911139e379f1e4ca813df1778b4ef"

    fetcher = WeatherBallparkRunFetcher(API_KEY)

    print("\n[INFO] RUN WEATHER & BALLPARK ENRICHMENT")
    print("="*80)
    print("[INFO] This will add weather and ballpark factors to Run metrics")
    print("="*80)

    # Run final cascading enrichment
    input_file = "run_metrics_output.json"
    if os.path.exists(input_file):
        print(f"\n[INFO] Found {input_file}, starting final enrichment...")
        final_data = fetcher.enrich_lineups_with_weather(input_file, "final_integrated_run_data.json")
        if final_data:
            print(f"\n[SUCCESS] RUN INTEGRATION PIPELINE COMPLETE!")
            print(f"[INFO] Final output: final_integrated_run_data.json")
            print(f"[INFO] Ready for Run prediction model!")
    else:
